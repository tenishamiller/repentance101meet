...
export function formatMessageThreadDeleteCountdown(purgeAt: Date | string) {
  ...
  const minutes = Math.floor((ms % (60 * 60 * 1000)) / (60 * 60 * 1000));
  if (hours > 0) return `${hours}h ${minutes}m`;
  return `${minutes}m`;
}

export function isMessageThreadRestorable(thread: {
  deletedAt?: Date | string | null;
  purgeAt?: Date | string | null;
  permanentlyDeletedAt?: Date | string | null;
}) {
  if (thread.permanentlyDeletedAt) return false;
