...
type UsePrivateMinistrySessionOptions = {
  meetingToken: string;
  meetingTitle: string;
  userId: string;
  userName: string;
...
export function usePrivateMinistrySession({
  meetingToken,
  meetingTitle,
  userId,
  userName,
  ...
  onMeetingEnded,
  onRecordingSaved,
}: UsePrivateMinistrySessionOptions) {
  const isPrivate = true;
  const mode = "private" as "livestream" | "private";
  const publishMedia = true;
